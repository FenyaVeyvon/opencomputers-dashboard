...
function M.new(config, log)
  ...
  local function baseUrl(self)
    return "http://" .. hostOnly(self.config.host)
  end

  function net:connect()
    local ok, handle = pcall(component.internet.request, baseUrl(self) .. "/api/health")
  ...
  function net:send(msg)
    ...
    local url = baseUrl(self) .. "/api/nodes/" .. node .. "/rpc"
