...
function M.new(config, log)
  ...
  function net:connect()
    self:close()
    local ok, sock = pcall(component.internet.connect, hostOnly(self.config.host), self.config.port)
    if ok and sock then
      self.socket = sock
      return true
    end
  ...
  function net:send(msg)
    ...
    if not ok then ok, err = pcall(function() return self.socket.write(self.socket, data) end) end
    if not ok then
      self:close()
      return nil, err
    end
  ...
  function net:poll()
    ...
    if not ok then ok, chunk = pcall(function() return self.socket.read(self.socket, 4096) end) end
    if not ok then
      self:close()
      return nil
    end
